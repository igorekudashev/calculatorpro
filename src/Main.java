...
public class Main {
    ...
    public static String calc(String input) {
        ...
        try {
            if (Integer.parseInt(parts[0]) < 1 || Integer.parseInt(parts[0]) > 10 || Integer.parseInt(parts[2]) < 1 || Integer.parseInt(parts[2]) > 10) throw new IllegalArgumentException();
            return String.valueOf(operations.get(parts[1]).applyAsInt(Integer.parseInt(parts[0]), Integer.parseInt(parts[2])));
        } catch (NumberFormatException ignored) {
            if (!romans.contains(parts[0]) || !romans.contains(parts[2])) throw new IllegalArgumentException();
            return arabicToRomanWrapper.get(0).apply(operations.get(parts[1]).applyAsInt(romans.indexOf(parts[0]) + 1, romans.indexOf(parts[2]) + 1));
        }
    }
}